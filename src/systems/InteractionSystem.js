...
class InteractionSystem {
   constructor(camera, scene, gameManager, uiManager, controls) {
    ...
    animateGarageDoorOpen(pivotObject, doorGroup, open = true) {
        ...
        const physicsBodyEntry = this.gameManager.mansion.physicsBodies.find(entry => entry.mesh === doorMesh);
        ...
        if (open) { // Remove physics BEFORE opening
            if (physicsBodyEntry?.body) {
                ...
                const index = this.gameManager.mansion.physicsBodies.findIndex(entry => entry.mesh === doorMesh);
                if (index !== -1) this.gameManager.mansion.physicsBodies.splice(index, 1);
            } else {
        ...
        log.log(`   - Animation Start: Pivot Y=${startRotationY.toFixed(2)}, Target Y=${targetRotationY.toFixed(2)}`);
         ...
         if (Math.abs(pivotObject.rotation.y - targetRotationY) < 0.01) {
             ...
             if (!open) {
                 if (this.gameManager.mansion?.recalculatePhysicsForObject) {
                    this.gameManager.mansion.recalculatePhysicsForObject(doorMesh.name);
        ...
        if (intersects.length > 0) {
            ...
            if (distance <= this.interactionRange) {
                ...
                if (data) {
                    ...
                    const isItemNotReady = (type === 'diary' || type === 'fireplace' || type === 'bucket' || type === 'fuse_box' || type === 'crowbar' || type === 'toolbox' || type === 'gas_can_part' || type === 'car_engine_zone') && interactableData.interactable === false;
                    ...
                    if (isPagesLocked) {
                        ...
                        if (typeInfo) {
                            ...
                            if (type === 'page_slot') {
                                ...
                                const garageDoor = this.scene.getObjectByName('S_Door002'); // Get the door object
